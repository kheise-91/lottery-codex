...
export default function BottomNavTabs({ onChange }) {
  return (
    <Tab.Group defaultIndex={0} onChange={(idx) => onChange?.(idx)}>
      <Tab.List className="fixed bottom-0 left-0 right-0 z-50 hidden h-14 flex-row items-center justify-around bg-white border-t border-gray-200 md:flex" style={{ boxShadow: '0 -2px 12px rgba(0,0,0,0.06)' }}>
        <Tab className={({ selected }) => `relative flex-1 flex flex-col items-center justify-center gap-0.5 py-2 cursor-pointer transition-colors duration-150 ${selected ? 'text-[var(--color-primary)]' : 'text-gray-400'}`}>
          {selected && (
