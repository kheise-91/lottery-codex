...
Slim Framework 4 REST API serving JSON endpoints. The API entry point (`backend/api.php`) is **not yet implemented** -- the routing layer and controllers do not exist. This document describes the planned endpoint contract based on `GameInterface` method signatures.
...
|----------|----------------------|-------|
| GET `/api/games` | Not implemented | `backend/api.php` does not exist |
| GET `/api/games/{gameId}` | Not implemented | Same -- routing layer missing |
| GET `/api/games/{gameId}/history` | Not implemented | Game classes implement `getHistory()` but API entry point is absent |
| POST `/api/games/{gameId}/generate` | Not implemented | Game classes implement `generateTickets()` but API entry point is absent |
