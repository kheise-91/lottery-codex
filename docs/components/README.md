...
| [SkeletonLoader](./SkeletonLoader.md) | `frontend/src/components/SkeletonLoader.jsx` | Implemented | Reusable pulsing gray placeholder block (rounded rectangle or circle) for loading states; used by GamePage while history is loading or tickets are generating |
| [ErrorBanner](./ErrorBanner.md) | `frontend/src/components/ErrorBanner.jsx` | Implemented | Dismissible red error banner with a user-facing message prop and close button; used by GamePage in the drawings and ticket areas when API calls fail |
