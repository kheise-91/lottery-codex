...
1. **TicketCard** (default export) -- Top-level wrapper that maps over `tickets`, rendering a `<Ticket>` for each and inserting a `<Perforation>` divider between consecutive tickets.
2. **Ticket** -- Renders a single ticket card with header, panels section, and footer strip.
...
### TicketCard (top-level)

```jsx
<div className="space-y-0">
  {tickets.map((ticketData, index) => (
    <>
      <Ticket game={game} ticketData={ticketData} index={index} />
...
An `<article>` element with:
- **Header** (`px-5 pt-5 pb-2`): Game name (`<h2>`) and ticket ID on the left, decorative `<Barcode />` on the right.
...
- Panel badge: text color and semi-transparent background derived from the game color.
- Perforation divider: radial gradient dot pattern via `backgroundImage`.

No custom CSS classes are defined in this component or added to `index.css`.
