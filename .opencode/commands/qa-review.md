...
Confirm the `master` branch is not checked out:
...
Then, parse the output into a flat list of changed file paths and split that list by top-level path:
...
Keep the file list and diff content together per section - both get passed to the corresponding reviewer.
...
Spawn each reviewer agent required (one at a time), passsing the file list and diff content for their section, along with the following instructions:
