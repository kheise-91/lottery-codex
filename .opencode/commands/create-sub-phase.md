...
The sub-phase number is: $1. If no sub-phase number is passed, stop and ask the user for a sub-phase number before proceeding.
...
Read @ROADMAP.md. Find the sub-phase matching "$1" (e.g. "3.2" matches `- [ ] **3.2...**`). 
...
Using the Gitea MCP Server, detect the repo from the current git remote. Create a milestone with the information below, following the formatting exactly as shown:
