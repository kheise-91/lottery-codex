...
Spawn the @git-manager subagent and hand it the milestone spec and the ordered issue specs verbatim. It first makes sure the phase branch is in place: if `phase-X` is not checked out, it checks it out — if the branch does not exist yet, it creates it from `master` and pushes it to origin (this only happens for the first sub-phase of a phase — for later sub-phases it should already exist) — then it fetches from origin and pulls (skip fetch and pull for a newly created branch; just make sure it is pushed). It then creates the sub-phase branch `phase-X.Y` off `phase-X`, the milestone `Phase X.Y` (reusing it if it exists), and each issue (label `Task`, plan body, milestone attached). It returns the branch name, the milestone number/URL, and every issue number.

**Step 4 — Docs update (in progress).**
Spawn the @docs-manager subagent in **command mode** with the sub-phase number, title, and the milestone URL from Step 3. It marks the sub-phase in progress in ROADMAP.md: checkbox `[-]` and the title linked to the milestone URL (format per its `roadmap-entry` template). Then commit and push the change yourself with the message `[Phase-$1] Starting $1`.
