...
The issue number is: $1. If no issue number was provided by the user, stop and ask the user to provide one.

Read the saved issue plan: @.opencode/plans/issue-$1.md. The issue details provided in the saved issue plan will be used in the steps below. If no issue plan is found, stop and alert the user.
...
**Scope Boundary**
The implementation scope for this task is defined entirely by the contents of the saved issue plan - nothing else. Do not implement any work that falls outside the scope of the issue plan. When the verification steps and acceptance criteria from the issue plan are satisfied, the work is done.

---

# Step 1 - Spawn the engineer agents

Based on what the issue plan requires, spawn the appropriate agents listed below (one at a time):
...
- The full issue plan text - read `.opencode/plans/issue-$1.md` and include its entire contents verbatim in the agent prompt (do NOT summarize or truncate)
...
If a mockup file is referenced in the issue plan, instruct the `frontend-engineer` to analyze the mockup file:
...
Spawn only the agents the issue actually requires. A frontend-only issue skips the backend-engineer, backend-only issue skips frontend-engineer, etc.
...
- Read the entire issue plan provided by the orchestrator (`.opencode/plans/issue-$1.md`)
...
- Perform visual and interaction review using the Playwright MCP Server (`frontend-reviewer` only)
...
# Step 3 - Spawn the `docs-manager` agent

Spawn the @docs-manager agent, passing the following context and instructions:
...
**Instructions:**
You are reviewing the code changes made for this issue and updating the relevant documentation. You are in **scoped mode** - review ONLY the diff content provided - do not review the full file. Remember to stick to your scope limitation and update only documentation relevant to the code changes provided.

Wait for the `docs-manager` agent to complete before proceeding.
...
Open a pull request via the Gitea MCP Server:
